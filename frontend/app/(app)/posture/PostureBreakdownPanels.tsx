      ...
      <div className="mt-4 flex items-center gap-6">
        ...
        <div className="flex flex-col gap-1">
          {segments.map((seg) => (
            <Link
              key={seg.key}
              href={findingsHref({ severity: seg.key })}
              aria-label={`View ${seg.key} findings`}
              className="-mx-2 flex items-center gap-2 rounded-md px-2 py-1 text-xs transition-colors hover:bg-[var(--color-surface-raised)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--color-accent)]"
            >
              <span
    ...
    <Card className="rounded-2xl">
      ...
      <div className="mt-4 space-y-1">
        {repos.map((repo) => (
          <Link
            key={repo.name}
            href={findingsHref({ repo: repo.name })}
