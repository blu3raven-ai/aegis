...
export function OrgGeneralSection() {
  ...
  const changeCount = data && draft && draft.name !== data.name ? 1 : 0
  ...
  return (
    <SettingsSection
      id="general"
      title="General"
      subtitle="Organization name and logo"
    >
      <SettingsCard heading="Identity">
        ...
        <SettingsRow label="Logo" description="PNG / SVG / JPEG · max 100 KB · square works best">
          {data?.logoDataUrl ? (
            ...
            <div
              aria-hidden="true"
              className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-accent-gradient-end)] text-xs font-bold text-white"
            >
              {initials}
            </div>
          )}
          ...
          {logoError && <span role="alert" className="text-xs text-[var(--color-status-error)]">{logoError}</span>}
        </SettingsRow>
      </SettingsCard>
    </SettingsSection>
  )
