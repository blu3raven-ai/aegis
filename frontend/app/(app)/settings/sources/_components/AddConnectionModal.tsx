...
export function AddConnectionModal({
  ...
  const availableTypes = CATEGORY_SOURCE_TYPES[category]
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-6 py-4">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold text-[var(--color-text-primary)]">
              {step === 1 ? `Add ${categoryLabel} Connection` : `Connect to ${selectedType ? SOURCE_TYPE_LABELS[selectedType] : ""}`}
            </h2>
          </div>
          <button
            type="button"
          ...
          {step === 1 ? (
            <>
              <button
                type="button"
                onClick={onClose}
                className="rounded-lg px-4 py-2 text-sm font-medium text-[var(--color-text-secondary)] transition-colors hover:text-[var(--color-text-primary)]"
              >
                Cancel
              </button>
              <button
