...
test("account settings renders row-based security controls", () => {
  ...
  const disableTotp = readFileSync(new URL("./DisableTotpModal.tsx", import.meta.url), "utf8")
  ...
  assert.match(account, /<DisableTotpModal/)
  // Disabling 2FA now re-authenticates with a current code before calling disable.
  assert.match(disableTotp, /\/api\/v1\/auth\/totp\/disable/)
  assert.match(disableTotp, /body:\s*\{\s*code/)
