...
// ── Home dashboard batched query types ───────────────────────────────────────

export interface GqlSeverityBreachStat {
  open: number
  breached: number
  breachedPct: number
}

export interface GqlBreachSummary {
  critical: GqlSeverityBreachStat
  high: GqlSeverityBreachStat
  medium: GqlSeverityBreachStat
  low: GqlSeverityBreachStat
}
...
export interface GqlSourceAuth {
  orgOrOwner: string
}

export interface GqlSourceConnection {
  id: string
  sourceType: string
  category: string
  name: string
...
export interface GqlSourceConnectionsResponse {
  connections: GqlSourceConnection[]
}

export interface GqlHomeDashboard {
  dependenciesCounts: GqlSeverityCounts
  codeScanningCounts: GqlSeverityCounts
  containerCounts: GqlSeverityCounts
  secretCounts: GqlSeverityCounts
  postureTrend: GqlPostureTrendPoint[]
  homeAnalytics: GqlHomeAnalytics
  slaBreachSummary: GqlBreachSummary
  epssTop: GqlEpssTopResponse
  sourceConnections: GqlSourceConnectionsResponse
}
