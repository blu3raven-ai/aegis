...
export const HOME_DASHBOARD_QUERY = `
  query HomeDashboard($trendDays: Int, $epssLimit: Int) {
    dependenciesCounts { total critical high medium low }
    codeScanningCounts { total critical high medium low }
    containerCounts { total critical high medium low }
    secretCounts { total critical high medium low }
    postureTrend(days: $trendDays) { date total critical high medium low }
    homeAnalytics {
      topRepositories { name open critical high }
      ageBuckets { label count }
      remediation { totalFixed avgDays medianDays fixedLast30d }
    ...
    slaBreachSummary {
      critical { open breached breachedPct }
      high { open breached breachedPct }
      medium { open breached breachedPct }
      low { open breached breachedPct }
    }
    ...
    sourceConnections {
      connections {
        id sourceType category name status lastSyncedAt
        auth { orgOrOwner }
      }
    }
  }
