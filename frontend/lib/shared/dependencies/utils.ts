...
export function findingIdentityKey(alert: DependenciesFinding): string {
  ...
  return `${repo}::${packageName}::${ecosystem}::${advisoryId}::${manifestPath}`
}
...
export interface DependenciesFilterState {
  ...
  ageBucket: string      // matches DEPENDENCIES_AGE_BUCKETS label
}

export const DEFAULT_DEPENDENCIES_FILTERS: DependenciesFilterState = {
  state: "open",
  severity: [],
  ecosystem: [],
  organization: "",
...
export const DEPENDENCIES_AGE_BUCKETS = [
...
// ── CVSS score range buckets ──────────────────────────────────────────────────

export const CVSS_RANGE_BUCKETS = [
  { label: "9.0+",    min: 9.0, max: 10.1 },
  { label: "7.0–8.9", min: 7.0, max: 9.0  },
  { label: "4.0–6.9", min: 4.0, max: 7.0  },
  { label: "0.1–3.9", min: 0.1, max: 4.0  },
] as const
...
/** Apply DependenciesFilterState to a flat findings array. Does not dedupe. */
export function filterFindings(
  ...
  return findings.filter((a) => {
    ...
    if (filters.ageBucket) {
      const bucket = DEPENDENCIES_AGE_BUCKETS.find((b) => b.label === filters.ageBucket)
      if (bucket) {
...
export function filterFindingsSimple(
  ...
  const ageBucket = opts.ageBucket
    ? DEPENDENCIES_AGE_BUCKETS.find((b) => b.label === opts.ageBucket) ?? null
    : null
  ...
  return findings.filter((a) => {
    ...
    if (opts.cvssRange) {
      const bucket = CVSS_RANGE_BUCKETS.find((b) => b.label === opts.cvssRange)
      if (bucket) {
        const score = a.security_advisory.cvss?.score ?? 0
        if (score < bucket.min || score >= bucket.max) return false
      }
    }
    if (opts.newSinceLastScan && opts.lastScanDate) {
      if (!a.first_seen_at || a.first_seen_at < opts.lastScanDate) return false
    }
