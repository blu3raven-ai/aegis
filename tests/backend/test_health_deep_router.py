...
PROBE_NAMES = ["postgres", "redis", "minio", "connected_runners", "recent_scans", "correlation_engine", "argus"]
...
class TestDeepHealthEndpoint:
    ...
    def test_overall_status_ok_when_all_ok_or_skipped(self):
        results = [_make_probe("postgres","ok"),_make_probe("redis","ok"),_make_probe("minio","ok"),
    ...
    def test_overall_status_fail_when_any_critical_fails(self):
        results = [_make_probe("postgres","fail","connection refused"),_make_probe("redis","ok"),
    ...
    def test_fail_takes_precedence_over_degraded(self):
        results = [_make_probe("postgres","fail","timeout"),_make_probe("redis","degraded"),
                   _make_probe("minio","ok"),_make_probe("connected_runners","skipped"),
    ...
    def test_response_contains_all_seven_probes(self):
        all_ok = [_make_probe(n, "ok") for n in PROBE_NAMES]
        app = _make_app()
        with patch("src.health.router.run_all_probes", new=AsyncMock(return_value=all_ok)):
            with TestClient(app) as c:
    ...
    def test_probe_error_is_reflected_in_response(self):
        ...
        results[1] = _make_probe("redis", "fail", "connection refused")
        app = _make_app()
        with patch("src.health.router.run_all_probes", new=AsyncMock(return_value=results)):
            with TestClient(app) as c:
                data = c.get("/health/deep").json()
        redis_probe = next(p for p in data["probes"] if p["name"] == "redis")
        assert redis_probe["status"] == "fail"
        assert redis_probe["error"] == "connection refused"
