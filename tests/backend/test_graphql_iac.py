"""Unit tests for the IAC GraphQL resolver.

Phase A surface is just `iacCounts`. Covers empty scope (fail-closed) and
...
from src.graphql.schema import IacScanningQuery
...
@pytest.mark.asyncio
async def test_iac_counts_empty_scope_returns_zeros(empty_scope_ctx):
    ...
    with patch(
        ...
        result = await IacScanningQuery().counts(_info())

    helper.assert_called_once_with([], tool="iac_scanning", state="open")
    assert result.total == 0
    assert result.critical == 0


@pytest.mark.asyncio
async def test_iac_counts_returns_helper_counts(scoped_ctx):
    ...
    helper.assert_called_once_with(["a1", "a2"], tool="iac_scanning", state="open")
...
@pytest.mark.asyncio
async def test_iac_counts_only_counts_iac_scanning_tool(scoped_ctx):
    ...
    with patch(
        ...
        await IacScanningQuery().counts(_info())

    assert called_with["tool"] == "iac_scanning"
