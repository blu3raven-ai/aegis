...
from src.argus.connector import Decision, Explanation, NullArgusConnector, RiskScore
...
def test_null_decide_go_no_go_returns_decision():
    connector = NullArgusConnector()
    result = connector.decide_go_no_go(
        "svc-1",
        [{"id": "f1", "severity": "low"}],
    )
...
def test_null_decide_go_no_go_no_network_calls():
    connector = NullArgusConnector()
    with patch("httpx.Client") as mock_client:
        connector.decide_go_no_go("svc-1", [])
    mock_client.assert_not_called()


def test_null_decide_allow_on_empty_findings():
    connector = NullArgusConnector()
    result = connector.decide_go_no_go("svc-1", [])
    assert result.decision == "allow"


def test_null_decide_warn_on_high_severity():
    connector = NullArgusConnector()
    result = connector.decide_go_no_go("svc-1", [{"id": "f1", "severity": "high"}])
    assert result.decision == "warn"


def test_null_decide_block_on_critical_severity():
    connector = NullArgusConnector()
    result = connector.decide_go_no_go("svc-1", [{"id": "f1", "severity": "critical"}])
    assert result.decision == "block"
    assert "f1" in result.blockers
