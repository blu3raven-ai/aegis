"""AgentScanner.run_scan — multi-repo dispatch, aggregation, and stamping.
...
import runner.scanners.agent.scanner as scanner_mod
from runner.scanners.agent.scanner import AgentScanner, AgentScanConfig


def _fake_clone(url, dest, token=None, timeout=None):
    """Materialize a checkout dir so scan_repo has something to walk."""
...
def test_run_scan_scans_every_repo_and_stamps_each(tmp_path, monkeypatch):
    ...
    def fake_scan_repo(clone_dir):
        return [{"check_id": "AGENT_TEST", "file": "AGENTS.md", "line": 1,
                 "severity": "high", "resource": clone_dir}]

    monkeypatch.setattr(scanner_mod, "scan_repo", fake_scan_repo)

    job = {"jobId": "t", "envVars": {
        "GIT_REPOS": "https://github.com/acme/a.git\nhttps://github.com/acme/b.git",
    }}
    job_dir = tmp_path / "job"
    result = AgentScanner().run_scan(job, job_dir=job_dir)

    assert result.exit_code == 0
    lines = _findings_lines(job_dir)
    # Both repos scanned — not just the first.
    repos = sorted(f["repo_full_name"] for f in lines)
    assert repos == ["a", "b"], f"expected both repos scanned, got {repos}"
    # Each finding carries its own repo's html url.
...
def test_run_scan_tolerates_partial_clone_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scanner_mod, "scan_repo", lambda d: [
        {"check_id": "AGENT_TEST", "file": "x", "line": 1, "severity": "low", "resource": "r"}])
    ...
    # The good repo still produces findings; a partial failure is not fatal.
    assert result.exit_code == 0
    lines = _findings_lines(job_dir)
