              ...
              <>
                        ...
                        {chain.length > 0 ? chain.map((step, idx) => {
                          ...
                          <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-raised)] px-3 py-2.5">
                            <div className="flex items-center gap-2">
                              <svg className="h-3.5 w-3.5 shrink-0 text-[var(--color-text-secondary)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                                <path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>
                              </svg>
                              <span className="text-xs font-semibold text-[var(--color-text-primary)]">Runs at module level</span>
                            </div>
                            <p className="mt-1 text-[11px] text-[var(--color-text-secondary)]">Executes on import — no explicit call site.</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {verdict === "unreachable" && (
                  <div className="rounded-lg border border-[var(--color-verdict-safe-border)] bg-[var(--color-verdict-safe-subtle)] px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <svg className="h-3.5 w-3.5 shrink-0 text-[var(--color-verdict-safe)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <circle cx="12" cy="12" r="10"/><path d="m4.9 4.9 14.2 14.2"/>
                      </svg>
                      <span className="text-xs font-semibold text-[var(--color-verdict-safe)]">Cannot be triggered</span>
                    </div>
                    <p className="mt-1 text-[11px] text-[var(--color-text-secondary)]">Not reachable from any detected entry point — lower exploitation risk.</p>
                  </div>
                )}
                {verdict === "unknown" && (
