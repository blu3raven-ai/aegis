...
const AGE_BUCKETS = [
  { label: "< 7d", key: "0-7d", colour: "bg-emerald-400" },
  { label: "7–30d", key: "8-30d", colour: "bg-amber-400" },
  { label: "30–90d", key: "31-90d", colour: "bg-orange-400" },
  { label: "90d+", key: "90d+", colour: "bg-red-400" },
]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <KpiCard label="Urgent" value={String(urgent)} note="Critical + High open" valueClass="text-red-400" onClick={() => onGoToFindings({ severity: "critical" })} />
        <KpiCard label="Awaiting fix" value={String(awaitingCount)} note="Fix in progress" valueClass="text-orange-400" onClick={() => onGoToFindings({ state: "awaiting_fix" })} />
        <KpiCard label="Stale (>30d)" value={String(staleCount)} note="Open and unpatched >30 days" valueClass="text-amber-400" onClick={() => onGoToFindings({ state: "open", ageBucket: "30d+" })} />
        <KpiCard label="Fixed recently" value={String(fixed30d)} note="Closed in last 30 days" valueClass="text-emerald-400" onClick={() => onGoToFindings({ state: "fixed" })} />
      </div>
            ...
            <div className="space-y-2">
              {topRules.slice(0, 5).map((rule) => (
                ...
                >
                  ...
                  <span className="shrink-0 rounded-full bg-orange-500/10 px-2.5 py-1 text-xs font-bold tabular-nums text-orange-400">
        ...
        <div className="flex flex-col rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[0_28px_80px_rgba(15,23,42,0.06)]">
            ...
            <div className="flex h-full">
              {counts.total > 0 && (
                <>
                  ...
                  <div className="bg-orange-400" style={{ width: `${(counts.high / counts.total) * 100}%` }} />
                  <div className="bg-amber-400" style={{ width: `${(counts.medium / counts.total) * 100}%` }} />
                  <div className="bg-blue-400" style={{ width: `${(counts.low / counts.total) * 100}%` }} />
                </>
              )}
            </div>
          </div>
          <div className="mt-4 space-y-2">
            {(["critical", "high", "medium", "low"] as const).map((sev) => {
              ...
              const tones: Record<string, string> = { critical: "bg-red-500", high: "bg-orange-500", medium: "bg-amber-400", low: "bg-blue-400" }
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[0_28px_80px_rgba(15,23,42,0.06)]">
          ...
          <div className="mt-4 rounded-2xl bg-[var(--color-surface-raised)] p-4">
            <p className="text-4xl font-semibold text-[var(--color-text-primary)]">{coverage?.percentage ?? 0}%</p>
        ...
        <div className="rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[0_28px_80px_rgba(15,23,42,0.06)]">
          ...
          <h3 className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">How Fast Issues Are Being Closed</h3>
...
function Card({ eyebrow, title, subtitle, children }: { eyebrow: string; title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col rounded-[28px] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[0_28px_80px_rgba(15,23,42,0.06)]">
      ...
      {title && <h3 className="mt-2 text-xl font-semibold text-[var(--color-text-primary)]">{title}</h3>}
