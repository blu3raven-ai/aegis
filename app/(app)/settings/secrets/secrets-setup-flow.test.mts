...
test("SecretsSetupForm.tsx includes status display logic", () => {
  const source = readFileSync(path.join(dir, "SecretsSetupForm.tsx"), "utf8")
  assert.match(source, /let status: "Setup required" \| "Verifying" \| "Ready" = "Setup required"/)
  assert.match(source, /status === "Setup required"/)
...
test("SecretsSetupForm.tsx has organization scope selection", () => {
  const source = readFileSync(path.join(dir, "SecretsSetupForm.tsx"), "utf8")
  assert.match(source, /Organization Scope/)
  assert.match(source, /orgScope === "all"/)
  assert.match(source, /orgScope === "selected"/)
})
...
test("settings tool forms include no-org empty-state guidance", () => {
  const contentSource = readFileSync(new URL("./SecretsContent.tsx", import.meta.url), "utf8")
  const sharedFormSource = readFileSync(new URL("../ToolSettingsForm.tsx", import.meta.url), "utf8")

  assert.match(contentSource, /No organizations configured yet\./)
  assert.match(sharedFormSource, /Settings > GitHub/)
})
...
test("SecretsSetupForm.tsx has AI Enhanced scan depth card", () => {
  ...
  assert.match(source, /BetterLeaks/)
