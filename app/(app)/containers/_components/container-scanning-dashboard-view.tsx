...
export function ContainerScanningDashboardView({ org, initialTab, canEdit, prerequisitesMet }: { org: string; initialTab?: string; canEdit?: boolean; prerequisitesMet?: boolean }) {
  ...
  if (error && !gqlAnalytics) {
    return (
      <div className="rounded-lg bg-[var(--color-severity-critical-subtle)] border border-[var(--color-severity-critical-border)] p-5 text-sm text-[var(--color-severity-critical)]">
      ...
      {showBanner && latestRun && (
        <div className="mb-6">
          <ScanRunningBanner
            ...
            nowMs={nowMs}
            commandLabel={`root@scanner:~$ ./run-container-scan.sh --org ${latestRun.org}`}
