...
export async function listAuditEvents(filters: AuditQueryFilters): Promise<AuditEventsResponse> {
  ...
  const res = await fetch(`/api/v1/audit/events?${qs.toString()}`, { cache: "no-store" })
  if (!res.ok) {
    const text = await res.text().catch(() => "")
    throw new Error(`audit-api: ${res.status} ${res.statusText} — ${text}`)
  }
  return res.json() as Promise<AuditEventsResponse>
}
