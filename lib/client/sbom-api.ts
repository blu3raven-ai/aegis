...
async function fetchText(url: string): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`sbom-api: ${res.status} ${res.statusText} — ${text}`)
  }
  return res.text()
}
...
export async function fetchSbomHistory(repoId: string, limit?: number): Promise<SbomHistoryEntry[]> {
  ...
  const text = await fetchText(`/api/v1/sboms/repo/${encodeURIComponent(repoId)}/history${query}`)
  const data = JSON.parse(text) as SbomHistoryEntry[] | { items: SbomHistoryEntry[] }
...
export async function fetchSbom(params: {
  ...
  if (repoId) {
    return fetchText(`/api/v1/sboms/repo/${encodeURIComponent(repoId)}?${qs.toString()}`)
  }
  if (imageDigest) {
    return fetchText(`/api/v1/sboms/image/${encodeURIComponent(imageDigest)}?${qs.toString()}`)
  }
