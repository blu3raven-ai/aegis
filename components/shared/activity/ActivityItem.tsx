...
function formatActor(actor: string | null): string | null {
  if (!actor || actor === "system" || actor === "argus") return null
  // Show only the local part of an email address for brevity.
  const atIndex = actor.indexOf("@")
  return atIndex > 0 ? actor.slice(0, atIndex) : actor
}
...
export function ActivityItem({ event }: ActivityItemProps) {
  ...
  const content = (
    <div
      className="group flex items-start gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-[var(--color-surface-raised)]"
