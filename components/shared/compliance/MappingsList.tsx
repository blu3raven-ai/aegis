...
export function MappingsList({ findings, emptyMessage = "No findings mapped to this control." }: MappingsListProps) {
  if (findings.length === 0) {
    return (
      <div className="flex items-center justify-center py-10 text-[13px] text-[var(--color-text-secondary)]">
        {emptyMessage}
      </div>
    )
  }
        ...
        <div key={finding.id} className="flex items-start justify-between gap-4 py-3">
          <div className="min-w-0 flex-1">
            <Link
              ...
              className="block truncate text-[13px] font-medium text-[var(--color-text-primary)] hover:underline"
            >
              {finding.title}
            </Link>
