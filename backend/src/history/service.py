"""History feed service — Phase 52.

Provides a unified read-side view of recent org events by aggregating
rows from several tables (findings, finding_events, scan_runs) into a
single timeline. Cursor-based pagination uses base64-encoded
(occurred_at, id, source) tuples so callers get stable pages without
offset drift as new events land.

Audit-driven signals (KEV adds, CVE intel, integration changes, SLA
breaches) are not folded into this feed — they surface in
`/settings/audit`. The `audit_events` table has no `asset_id` column,
so per-asset scoping isn't possible here; we narrow the contract
rather than leak org-wide rows to viewers with partial access.
"""
...
from src.db.models import Finding, FindingEvent, ScanRun
...
class HistoryService:
    ...
    async def _query(
        ...
        want_scans = _wants(types, {"scan.completed", "scan.failed", "scan.cancelled"})
    ...
    async def _query_scan_runs(
        ...
        stmt = select(ScanRun).where(
            ...
            ScanRun.status.in_(["completed", "failed", "error", "cancelled"]),
        )

        # Order and paginate by the same instant the event surfaces as
        # (occurred_at below falls back to started_at). Without the coalesce,
        # a cancelled/terminal row with a null finished_at sorts nulls-last and
        # drops out of cursor pages — e.g. PR force-push supersede cancels.
        ts_col = func.coalesce(ScanRun.finished_at, ScanRun.started_at)
        if since:
        ...
        for run in rows:
            if run.status == "completed":
                evt_type = "scan.completed"
            elif run.status == "cancelled":
                evt_type = "scan.cancelled"
            else:
                evt_type = "scan.failed"
            if types and evt_type not in types:
                continue
