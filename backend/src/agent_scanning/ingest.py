"""Read agent-scanning findings.jsonl emitted by the runner.
...
def read_agent_findings(findings_path: Path) -> list[dict[str, Any]]:
    if not findings_path.exists():
        logger.warning("No agent findings.jsonl found at %s", findings_path)
        return []
