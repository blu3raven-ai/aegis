"""Agent-scanning finding lifecycle hooks for the shared lifecycle engine.
...
line number. The check_id (detector rule id) + resource pair distinguishes
multiple findings in one file; the line is mutable detail, not identity.
"""
...
def agent_finding_identity(repo: str, file: str, check_id: str, resource: str) -> str:
    """Stable, line-independent identity key for an agent-scanning finding."""
    def _esc(v: str) -> str:
...
class AgentScanningHooks(LifecycleHooks):
    tool = "agent_scanning"

    def compute_identity_key(self, raw: dict[str, Any]) -> str:
        return agent_finding_identity(
    ...
    def extract_detail(self, raw: dict[str, Any]) -> dict:
        detail: dict[str, Any] = {
            ...
            # Static per-rule advisory: the finding-detail endpoint reads these as
            # the drawer's description ("message") and remediation ("fixSuggestion").
...
agent_scanning_hooks = AgentScanningHooks()
