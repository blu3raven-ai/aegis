...
Receiver routes (``integrations/github|gitlab|bitbucket/webhook``) verify the
HMAC signature, normalize the body, then publish a typed ``code.push`` /
``code.pr_opened`` / ``code.pr_updated`` event via ``EventPublisher``. This
listener subscribes to that bus, resolves the canonical asset and forwards
the request to ``submit_ci_scan``.
...
class WebhookScanDispatcher:
    ...
    async def _dispatch_unchecked(self, event: SseEvent) -> None:
        ...
        # Pass org="" to match the CI router at trigger_router.py:78. The arg
        # is explicitly reserved/ignored today; keep the single call shape so
        # the eventual "wire org through" follow-up touches one site.
        ...
        # Mirror trigger_router.py:98 — push spam on a PR branch would otherwise
        ...
        # Mirror the audit shape from trigger_router.py:107 so webhook-driven
