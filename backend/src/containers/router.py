"""Container Scanning API endpoints.

Syft + Grype pipeline. Mirrors the SCA orchestration pattern:
start run -> runner job -> ingest SBOM + findings -> serve dashboard.
"""
...
from src.license.limits import get_tier
...
def get_history(request: Request, orgs: list[str] = Depends(require_orgs)) -> dict[str, Any]:
    ...
    for org_name in orgs:
        all_runs.extend(_enrich_run(r) or r for r in list_container_scanning_runs(org_name))
...
def get_latest_run(request: Request, orgs: list[str] = Depends(require_orgs)) -> dict[str, Any]:
    ...
    all_runs: list[dict[str, Any]] = []
    for org_name in orgs:
        all_runs.extend(list_container_scanning_runs(org_name))
    all_runs.sort(key=lambda r: r.get("createdAt", ""), reverse=True)
...
def start_runs(
    ...
    rate_limit_scan(request, "container_scanning")
    ...
    payload, status = start_multi_org_scan(
        orgs=orgs,
        runtime=_container_scanning_runtime,
        create_run_fn=create_container_scanning_run,
        execute_fn=execute_container_scan_once,
        execute_kwargs={"scanner_config": scanner_config, "mode": incremental_mode, "scan_mode": scan_mode},
        source_category="container-images",
...
def cancel_runs(
    ...
    return JSONResponse(cancel_multi_org_scan(orgs, _container_scanning_runtime, update_container_scanning_run, "container_scanning"))
