"""SCIM 2.0 routes — Users CRUD; Groups write deferred to v0.5.

Groups support is intentionally stubbed at 501 pending design for
multi-group conflict resolution (Aegis User has a single role_id) and
whether SCIM Groups create new Aegis roles or only map to existing ones.
"""
...
from src.auth.identity.auth import require_scim_auth
from src.auth.identity.schemas import ScimUser

scim_router = APIRouter(prefix="/scim/v2", tags=["auth"], dependencies=[Depends(require_scim_auth)])
...
def replace_user(request: Request, user_id: str, body: ScimUser) -> JSONResponse:
    ...
    if kind == "missing":
        return _scim_error(404, None, "User not found.")
    if kind == "not_managed":
        return _scim_error(409, "mutability", _NOT_SCIM_MANAGED_DETAIL)
    return JSONResponse(_to_scim(row, request), status_code=200)
...
def patch_user(request: Request, user_id: str, body: dict[str, Any]) -> JSONResponse:
    ...
    `op:"remove"` on the `active` path is decoded as soft-delete
    (status=deprovisioned) — same semantics as `op:"replace"` with
    `value=false`. The tombstone is kept; the row still exists with
    `active=false`. This matches the locked SCIM tombstone contract
    (rows are never hard-deleted via SCIM mutation).
    """
    ...
    async def _q(session):
        ...
        if not row.scim_managed:
            return ("not_managed", None)
        for op in ops:
            op_kind = (op.get("op") or "").strip().lower()
            path = (op.get("path") or "").strip()
            value = op.get("value")
            if op_kind == "remove" and path == "active":
