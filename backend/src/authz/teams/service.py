    ...
    async def _query(session):
        ...
        grant = (await session.execute(
            select(Grant).where(
                ...
                Grant.subject_id.in_(team_ids),
                Grant.asset_id == asset_row.id,
            )
        )).scalars().first()
...
def apply_github_sync_preview(preview: dict[str, Any], actor_user_id: str | None = None) -> None:
    ...
    async def _query(session):
        ...
        for t_create in preview.get("teamsToCreate", []):
            ...
            for r in t_create.get("repositoriesToAdd", []):
                ...
                if external_ref not in seen_asset_refs:
                    ...
                    await _upsert_team_grant(session, team_id=new_team.id, asset_id=asset_id, source="github")
        ...
        for t_update in preview.get("teamsToUpdate", []):
            ...
            # Remove grants by repo external_ref
            for r in t_update.get("repositoriesToRemove", []):
                ...
                if asset_row is not None:
                    await session.execute(
                        delete(Grant).where(
                            Grant.subject_type == "team",
                            Grant.subject_id == team.id,
                            Grant.asset_id == asset_row.id,
                        )
                    )

            # Add grants by repo external_ref
            for r in t_update.get("repositoriesToAdd", []):
                ...
                await _upsert_team_grant(session, team_id=team.id, asset_id=asset_id, source="github")

            # Remove grants by image external_ref
            ...
            # Add grants by image external_ref
