"""Deep-audit finding lifecycle hooks for the shared lifecycle engine.

Identity key: {repo}:{file}:{check_id}:{resource} — line-independent, so a
finding keeps its triage state when an unrelated edit shifts its line number.
The resource is the endpoint (e.g. "POST /api/x/{id}"), which distinguishes
multiple findings in one router file; the line is mutable detail, not identity.
"""
...
def deep_audit_identity(repo: str, file: str, check_id: str, resource: str) -> str:
    """Stable, line-independent identity key for a deep-audit finding."""
    def _esc(v: str) -> str:
