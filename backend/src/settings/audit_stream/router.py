...
from src.settings.general.schemas import AuditStreamConfigRequest

audit_stream_router = APIRouter(prefix="/api/v1/settings/audit/stream", tags=["settings"])
...
def patch_audit_stream(
    request: Request,
    body: AuditStreamConfigRequest,
    _: None = Depends(Permission(MANAGE_SETTINGS)),
) -> JSONResponse:
    async def _q(session):
