...
def _serialize(row: SsoConfig, request: Request) -> dict:
    ...
    return {
        ...
        "samlValidateMetadataSignature": row.saml_validate_metadata_signature,
...
@sso_router.patch("")
@audited(action="sso.config_updated", resource_type="sso_config")
def patch_sso(
    request: Request,
    ...
    async def _q(session):
        ...
        if body.samlValidateMetadataSignature is not None:
            row.saml_validate_metadata_signature = body.samlValidateMetadataSignature
...
@audited(action="sso.saml_keypair_generated", resource_type="sso_config")
def generate_saml_keypair(
    request: Request,
    _: None = Depends(Permission(MANAGE_SETTINGS)),
) -> JSONResponse:
...
@audited(action="sso.saml_metadata_refreshed", resource_type="sso_config")
