"""Scheduled IaC scan dispatch (execute_iac_scan_once).

Mirrors the code-scanning scheduled path: one iac_scanning runner job across the
org's repos. Crucially the job must carry SOURCE_TYPE so the backend ingest can
resolve assets (a gap the code-scanning scheduled path has).
"""
...
from src.iac import scanner as iac_scanner  # noqa: E402
...
def _patch(monkeypatch, *, sources, job_status="completed", job_error=None):
    ...
    monkeypatch.setattr("src.shared.config.get_scan_sources_for_org", lambda org: sources)
    monkeypatch.setattr("src.runner.jobs.create_job", fake_create_job)
    monkeypatch.setattr("src.runner.jobs.read_job", fake_read_job)
    monkeypatch.setattr(iac_scanner, "update_iac_run", fake_update)
...
def test_execute_dispatches_iac_job_with_source_type(monkeypatch):
    created, _ = _patch(monkeypatch, sources=[_src(["https://github.com/acme/api"])])
    out = iac_scanner.execute_iac_scan_once("acme", "tok", "run-1", source_type="github")
    assert out and out["org"] == "acme"
    assert created["job_type"] == "iac_scanning"
...
def test_execute_no_sources_completes_without_job(monkeypatch):
    ...
    out = iac_scanner.execute_iac_scan_once("acme", "tok", "run-2", source_type="github")
...
def test_execute_runner_failure_marks_failed(monkeypatch):
    ...
    out = iac_scanner.execute_iac_scan_once("acme", "tok", "run-3", source_type="github")
    assert out is None
