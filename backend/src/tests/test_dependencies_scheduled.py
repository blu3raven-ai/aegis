"""Scheduled dependency-scan dispatch (execute_dependencies_scan_once).
...
from src.dependencies import scanner as deps  # noqa: E402
...
def _patch(monkeypatch, *, sources, job_status="completed", job_error=None):
    ...
    monkeypatch.setattr(deps, "get_scan_sources_for_org", lambda org: sources)
    monkeypatch.setattr("src.runner.jobs.create_job", fake_create_job)
    monkeypatch.setattr("src.runner.jobs.read_job", fake_read_job)
    monkeypatch.setattr(deps, "update_dependencies_run", fake_update)
...
def test_execute_dispatches_deps_job_with_source_type(monkeypatch):
    created, _ = _patch(monkeypatch, sources=[_src(["https://github.com/acme/api"])])
    out = deps.execute_dependencies_scan_once("acme", "tok", "run-1", source_type="github", scanner_config={})
    assert out and out["org"] == "acme"
    assert created["job_type"] == "dependencies"
...
def test_execute_no_sources_completes_without_job(monkeypatch):
    ...
    out = deps.execute_dependencies_scan_once("acme", "tok", "run-2", source_type="github", scanner_config={})
...
def test_execute_runner_failure_marks_failed(monkeypatch):
    ...
    out = deps.execute_dependencies_scan_once("acme", "tok", "run-3", source_type="github", scanner_config={})
    assert out is None
